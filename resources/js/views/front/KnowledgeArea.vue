<template>
  <v-col xl="7" lg="8" md="12" order="3" order-lg="2">
    <h5 class="text-uppercase font-weight-regular py-2">Области знаний</h5>
    <p>В этой рубрике термины и комментарии специалистов классифицированы<br/>более развернуто по группам и
      направлениям.</p>
    <v-col sm="12" md="9" lg="6" class="d-flex align-center pl-0 mb-4">
    ...
    <v-col
      cols="12"
      v-if="loading"
      class="d-flex justify-center order-md-1"
    >
      <v-progress-circular
        width="5"
        size="48"
    ...
    <v-col cols="12" v-else class="px-0 order-sm-2">
      <v-card
        flat
        :key="i"
      ...
      >
        <v-card-title class="d-flex py-3">
          <v-col cols="12" class="pa-0 text-start w-100">
            <h5 class="mb-0 text-uppercase font-weight-regular">{{ knowledgeArea.name }}</h5>
          </v-col>
          <v-col lg="12" class="list-col-4 px-0">
            <v-card-text
              class="pa-0 mb-1"
