<template>
  <v-col xl="5" lg="6">
    <h5 class="text-uppercase font-weight-regular py-4">{{ category.name }}</h5>
    ...
    <div v-for="(item, i) in category.categoriables.data" :key="i">
      <quote-item
        ...
        :quote="item"
      ></quote-item>
      <term-item v-if="category.type == 'App\\Term'" :term="item"></term-item>
      <video-item
...
export default {
  ...
  data() {
    return {
      category: this.categoriables,
    };
  },
  created() {
    for (const category of this.category.categoriables.data) {
